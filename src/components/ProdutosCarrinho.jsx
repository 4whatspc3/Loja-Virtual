...
const ProdutosCarrinho = ({itemDoCarrinho, addShopCart, removeShopCart}) => {    
    return (
            <li className="flex flex-col gap-4 rounded-4xl rounded-b-none p-4 pt-0 border-b-2 border-sky-700">
                <header className="flex gap-2">
                    <figure className="w-full p-2 rounded-2xl">
                        <img style={{width : '50px', height: 'auto'}} src={itemDoCarrinho.image} alt={itemDoCarrinho.description} className="mx-auto w-40 h-40 object-contain"/>
                    </figure>
                    <h2 className="font-medium font-inter text-sky-800 ">{itemDoCarrinho.title}</h2>
                </header>
                <footer className="flex gap-4">
                        <div className="flex flex-col justify-between font-medium font-inter text-sky-800">
